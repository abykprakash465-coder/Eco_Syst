...
class DesktopScanner(QThread):
    ...
    @classmethod
    def scan(cls, cat_width: int, cat_height: int) -> list[DesktopItem]:
        ...
        has_gio = shutil.which("gio") is not None
        metadata_map = {}
        if has_gio and item_paths:
            try:
    ...
    def run(self) -> None:
        if self.isInterruptionRequested():
            return
        items = self.scan(self.cat_width, self.cat_height)
...
class DesktopCat(QWidget):
    ...
    def __init__(
        self,
        frames: dict[Animation, FrameSet],
        ...
        self.base_folder_mask = QRegion()
        self._last_masked_cat_rect: QRect | None = None
        ...
        initial_items = DesktopScanner.scan(self.CAT_WIDTH, self.CAT_HEIGHT)
        if initial_items:
            self.items = initial_items
            floor = float(screen.height() - self.CAT_HEIGHT)
            item_platforms = [it.platform for it in initial_items if it.platform.top() < floor - 30]
            self.platforms = list(self.manual_platforms) + item_platforms

        self.apply_folder_mask()
        self.show()
    ...
    def setup_tray(self) -> None:
        ...
        mask_menu = self.tray_menu.addMenu("📁 Folder Masking")
        self.mask_action_group = QActionGroup(self)
        self.mask_actions: dict[str, QAction] = {}

        folders_action = QAction("📁 Mask Desktop Folders", self, checkable=True)
        folders_action.setChecked(self.mask_mode == "folders")
        folders_action.triggered.connect(lambda: self.set_mask_mode("folders"))
        self.mask_action_group.addAction(folders_action)
        mask_menu.addAction(folders_action)
        self.mask_actions["folders"] = folders_action

        all_action = QAction("📄 Mask All Desktop Items", self, checkable=True)
        all_action.setChecked(self.mask_mode == "all")
        all_action.triggered.connect(lambda: self.set_mask_mode("all"))
        self.mask_action_group.addAction(all_action)
        mask_menu.addAction(all_action)
        self.mask_actions["all"] = all_action

        none_action = QAction("🚫 Disable Masking", self, checkable=True)
        none_action.setChecked(self.mask_mode == "none")
        none_action.triggered.connect(lambda: self.set_mask_mode("none"))
        self.mask_action_group.addAction(none_action)
        mask_menu.addAction(none_action)
        self.mask_actions["none"] = none_action
    ...
    def toggle_click_through(self, enabled: bool) -> None:
        ...
        self.show()
        if hasattr(self, "base_folder_mask") and not self.base_folder_mask.isNull():
            self.update_window_mask(force=True)
        if enabled:
    ...
    def apply_folder_mask(self) -> None:
        ...
        base = QRegion(screen_rect)
        if self.mask_mode == "folders":
            target_items = [it for it in self.items if getattr(it, "is_dir", True)]
            if not target_items:
        ...
        for it in target_items:
            base = base.subtracted(QRegion(it.icon_rect))

        self.base_folder_mask = base
        self.update_window_mask(force=True)

    def update_window_mask(self, force: bool = False) -> None:
        if self.mask_mode == "none":
            return

        if not hasattr(self, "base_folder_mask") or self.base_folder_mask.isNull():
            return

        cat_rect = QRect(int(self.pos_x), int(self.pos_y), self.CAT_WIDTH, self.CAT_HEIGHT)
        if (
            not force
            and self._last_masked_cat_rect == cat_rect
            and not self.treats
            and not self.hearts
            and self.speech.life <= 0
        ):
            return

        self._last_masked_cat_rect = cat_rect
        active_region = QRegion(cat_rect)

        if self.speech.life > 0 and self.speech.text:
            metrics = self.fontMetrics()
            tw = metrics.horizontalAdvance(self.speech.text)
            th = metrics.height()
            ...
            active_region = active_region.united(
                QRegion(QRect(bx - 10, by - 5, tw + 20, th + 10))
            )

        for treat in self.treats:
            active_region = active_region.united(
                QRegion(QRect(int(treat.x) - 10, int(treat.y) - 10, 20, 20))
            )

        for heart in self.hearts:
            active_region = active_region.united(
                QRegion(QRect(int(heart.x) - 10, int(heart.y) - 10, 20, 20))
            )

        full_mask = self.base_folder_mask.united(active_region)
        self.setMask(full_mask)

    def set_mask_mode(self, mode: str) -> None:
        if mode not in ("folders", "all", "none"):
            return
        self.mask_mode = mode
        if hasattr(self, "mask_actions") and mode in self.mask_actions:
            self.mask_actions[mode].setChecked(True)
        self.apply_folder_mask()
        if mode == "folders":
            self.say("Masking: Folders 📁", 2.0)
        elif mode == "all":
            self.say("Masking: All Items 📄", 2.0)
        else:
            self.say("Masking: Disabled 🚫", 2.0)
    ...
    def advance_physics(self, dt: float) -> None:
        ...
        if has_ctrl and not has_shift:
            ...
            clamped_x = max(20, min(cursor.x(), self.width() - 20))
            clamped_y = max(20, min(cursor.y(), self.height() - 20))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--mask-mode",
        choices=["folders", "all", "none"],
        default="all",
        help="Desktop folder mask mode (default: all)",
    )
...
def main() -> int:
    ...
    try:
        ...
        cat = DesktopCat(frames, args.platform, mask_mode=args.mask_mode)
    except ValueError as error:
